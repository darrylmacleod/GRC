...
class RiskRegisterApp:
    def __init__(self, root):
        ...
        tk.Button(actions_frame, text="Load Risks from Excel", command=self.load_from_excel).grid(row=0, column=4, padx=5, pady=5)
        tk.Button(actions_frame, text="Show Risk Chart", command=self.show_risk_chart).grid(row=0, column=5, padx=5, pady=5)

        # --- Treeview Table with Scrollbars ---
        table_frame = ttk.Frame(root)
        table_frame.grid(row=3, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")
    ...
    def show_risk_chart(self):
        ...
        self.chart_canvas.get_tk_widget().grid(row=4, column=0, columnspan=2, pady=10)
