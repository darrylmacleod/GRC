...
# --- Logging setup ---
logging.basicConfig(filename='dataclassifier.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
class ClassificationMap:
    ...
    def __init__(self):
        ...
        self.classification_levels: List[str] = ["Public", "Internal", "Confidential", "Restricted"]
    ...
    def import_excel(self, file_path: str) -> None:
        ...
        ws = wb.active
        for i, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):  # skip header
    ...
    def get_classification(self, data_type: str) -> str:
        """Get classification for a data type, or fuzzy match."""
        result = self.classification_map.get(data_type)
        if result:
            return result
        # Fuzzy matching
        matches = get_close_matches(data_type, self.classification_map.keys(), n=1, cutoff=0.6)
        if matches:
...
class DataClassifierApp:
    ...
    def __init__(self, root: tk.Tk, cmap: ClassificationMap):
        ...
        ttk.Entry(add_frame, textvariable=self.new_data_var, width=40).grid(row=0, column=0, sticky="EW", pady=(0, 8))
        ...
        ttk.Button(add_frame, text="Add Data Type", command=self.add_custom_type).grid(row=2, column=0, sticky="EW")
    ...
    def classify_data(self, event=None) -> None:
        ...
        if classification == "Unknown":
            ...
            self.result_label.configure(
                ...
                foreground=self.get_color(classification)
            )
    ...
    def add_custom_type(self) -> None:
        ...
        if self.cmap.add_classification(new_data, new_class):
            ...
            self.result_label.configure(
                ...
                foreground=self.get_color(new_class)
            )
            messagebox.showinfo("Added", f"Data type '{new_data}' with classification '{new_class}' added.")
            self.new_data_var.set("")
            self.new_classification_var.set("")
        else:
            messagebox.showerror("Error", "Failed to add the data type.")
    ...
    def import_csv(self) -> None:
        ...
        if file_path:
            try:
                ...
                self.dropdown['values'] = self.cmap.get_all_types()
                self.new_class_dropdown['values'] = self.cmap.get_levels()
                messagebox.showinfo("Imported", f"Classification data loaded from {file_path}")
    ...
    def get_color(self, level: str) -> str:
        """Return color for a classification level."""
        if self.theme == "dark":
            return {
                "Public": "#55D46A",
                "Internal": "#6AC6FF",
    ...
    def show_help(self) -> None:
        ...
        help_text = (
            ...
            "- Use the buttons at the bottom to import/export data (CSV or Excel).\n"
    ...
    def apply_theme(self) -> None:
        ...
        if self.theme == "dark":
            ...
            self.main.configure(style="Dark.TFrame")
            for widget in self.root.winfo_children():
                try:
                    widget.configure(bg="#232323")
                except Exception:
                    pass
        else:
