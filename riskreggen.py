...
# Download Sun Valley theme: https://github.com/rdbende/Sun-Valley-ttk-theme
...
# Load Sun Valley theme (optional)
def set_theme(root):
    try:
        root.tk.call("source", "sun-valley.tcl")
        ttk.Style().theme_use("sun-valley-light")
    except Exception:
        ttk.Style().theme_use("clam")  # fallback
...
class RiskRegisterApp:
    def __init__(self, root):
        ...
        # --- Input Frames ---
        details_frame = ttk.LabelFrame(root, text="Risk Details", padding=(16,12))
        details_frame.grid(row=0, column=0, padx=16, pady=(14,7), sticky="ew", columnspan=2)
        owner_frame = ttk.LabelFrame(root, text="Owner & Notes", padding=(16,12))
        owner_frame.grid(row=1, column=0, padx=16, pady=7, sticky="ew", columnspan=2)
        ...
        actions_frame.grid(row=2, column=0, padx=16, pady=12, sticky="ew", columnspan=2)
        ...
        ttk.Button(actions_frame, text="Export to Excel", command=self.export_to_excel).grid(row=0, column=3, padx=6, pady=4)
        ttk.Button(actions_frame, text="Export to CSV", command=self.export_to_csv).grid(row=0, column=4, padx=6, pady=4)
        ttk.Button(actions_frame, text="Load Risks from Excel", command=self.load_from_excel).grid(row=0, column=5, padx=6, pady=4)
        ttk.Button(actions_frame, text="Load Risks from CSV", command=self.load_from_csv).grid(row=0, column=6, padx=6, pady=4)
        ttk.Button(actions_frame, text="Show Risk Chart", command=self.show_risk_chart).grid(row=0, column=7, padx=6, pady=4)
        ttk.Button(actions_frame, text="Export Chart as PNG", command=self.export_chart_png).grid(row=0, column=8, padx=6, pady=4)
        ...
        table_frame.grid(row=4, column=0, columnspan=2, padx=16, pady=12, sticky="nsew")
